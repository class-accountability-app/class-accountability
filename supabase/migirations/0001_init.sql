...
-- A table with RLS disabled is readable by anyone with the anon key — which is
-- shipped to the browser. That is not a theoretical risk; it is the single most
-- common way a Supabase app leaks data, and it looks completely fine while you
-- test as yourself.
--
...
-- You can read the profile of anyone in a class you're also in.
-- (Needed to show podmates' names. Scoped to classes, not the whole user table.)
...
-- -----------------------------------------------------------------------------
-- Helper: is this user a podmate of mine?
--
-- This predicate is the heart of the app's privacy model, so it lives in ONE
-- place. Every "can I see this?" policy calls it. If the rule ever changes,
-- it changes here — not in six copy-pasted subqueries that drift apart.
--
-- SECURITY DEFINER + a locked search_path: the function reads pairing_members
-- with the definer's rights, avoiding infinite recursion when a policy on
-- pairing_members would otherwise need to query pairing_members.
-- -----------------------------------------------------------------------------
...
-- Without this, a new user exists in auth.users but has no profile, and every
-- policy above that joins to profiles quietly returns nothing.
-- =============================================================================
...
--
-- Run this query. Every row must show rls_enabled = true. If any row says
-- false, that table is public and you have a leak.
--
